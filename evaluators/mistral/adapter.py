...
def main() -> int:
    ...
    for path in sorted(assets.iterdir()):
        if (
            normalization_profile in {"wiki-v1", "wiki-v2", "wiki-v3", "wiki-v4", "wiki-v5"}
        ...
        **(
            {
                "additional_tools": [
                    {"name": "pypdf", "version": version("pypdf")},
                    {
                        ...
                        "version": (
                            "5.0.0" if normalization_profile == "wiki-v5" else "4.0.0" if normalization_profile == "wiki-v4" else "3.0.0" if normalization_profile == "wiki-v3" else "2.0.0"
                            if normalization_profile in {"wiki-v2", "wiki-v3", "wiki-v4", "wiki-v5"}
                            else "1.0.0"
                        ),
                    }
                ],
