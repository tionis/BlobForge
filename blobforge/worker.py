            ...
            def _report_progress(self):
                ...
                try:
                    ...
                    progress_data = {
                        ...
                        "tqdm_percent": round(100 * self.n / self.total, 1)
                        if self.total
                        else 0,
                        "tqdm_rate": round(self.format_dict.get("rate", 0) or 0, 2),
                        "tqdm_elapsed": round(
                            self.format_dict.get("elapsed", 0) or 0, 1
                        ),
                    }
...
class HeartbeatThread(threading.Thread):
    ...
    def __init__(
        self, s3_client: S3Client, worker_id: str, db: Optional[DatabaseClient] = None
    ):
    ...
    def run(self):
        ...
        while self.running:
            ...
            if job:
                ...
                # Also update job progress in database
                if self.db and self.db.available:
                    try:
                        self.db.update_job_progress(job, progress_data)
                    except Exception as e:
                        logger.debug(f"DB heartbeat update failed: {e}")

            # Also update worker heartbeat with system metrics
            self.s3.update_worker_heartbeat(
                current_job=job, system_metrics=get_system_metrics()
            )
            if self.db and self.db.available:
                try:
                    ...
                    logger.debug(f"DB worker heartbeat failed: {e}")
...
class Worker:
    ...
    def __init__(self, s3_client: S3Client):
        ...
        # Register worker (DB preferred, S3 fallback)
        ...
        self.heartbeat = HeartbeatThread(s3_client, self.id, self.db)
    ...
    def cleanup_previous_session(self):
        ...
        """
        logger.info(
            f"Worker {self.id}: Checking for stale locks from previous session..."
        )
        ...
        for job in jobs:
            ...
            if data.get("worker") == self.id:
                ...
                todo_retries = (
                    _safe_int(todo_data.get("retries", 0))
    ...
    def acquire_job(self) -> Optional[str]:
        ...
        When the database is available, uses PostgreSQL's
        SELECT ... FOR UPDATE SKIP LOCKED for atomic, race-free
        job acquisition. Falls back to S3-based locking otherwise.

        Returns the job hash if acquired, None otherwise.
        """
        # Try database first (atomic claim)
        if self.db.available:
            file_hash = self.db.claim_job(self.id)
            if file_hash:
                # Ensure file exists in DB manifest
                job = self.db.get_job(file_hash)
                if job:
                    self.current_job = file_hash
                    self.current_priority = job.get("priority", DEFAULT_PRIORITY)
                    self.heartbeat.set_job(file_hash)
                    self._empty_poll_count = 0
                    logger.info(
                        f"Claimed job {file_hash[:12]}... via database (priority={self.current_priority})"
                    )
                    return file_hash
            self._empty_poll_count += 1
            return None

        # Fall back to S3-based acquisition
        ...
        for priority in PRIORITIES:
            ...
            for job_hash in todos:
                ...
                if self.s3.acquire_lock(job_hash, self.id, priority):
                    ...
                    logger.info(
                        f"Lock acquired for {job_hash[:12]}... (priority={priority}, retry={retry_count}, file={original_name})"
                    )
                    ...
                    if lock_data:
                        ...
                        self.s3.put_object(
                            f"{S3_PREFIX_PROCESSING}/{job_hash}", json.dumps(lock_data)
                        )
    ...
    def process(self, job_hash: str):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                if self.s3.mock:
                    ...
                    md_text, images, marker_meta = self._run_conversion_with_timeout(
                        pdf_path, timeout_seconds=conversion_timeout
                    )
                    ...
                    logger.info(
                        f"Marker conversion completed: {len(md_text)} chars, {len(images)} images, {page_count or '?'} pages"
                    )
            except Exception as e:
            ...
            logger.info(
                f"Job {job_hash} Complete in {self.heartbeat._format_duration(processing_time)}."
            )

    def _complete_job(
        self, job_hash: str, file_size: int = 0, processing_time: float = 0
    ):
        """
        ...
        - Mark done in database if available
        """
        # Update throughput metrics
    ...
    def get_throughput_metrics(self) -> dict:
        ...
        avg_time = (
            self._total_processing_time / self._jobs_completed
        ...
        return {
            ...
            "bytes_processed_formatted": self.heartbeat._format_size(
                self._bytes_processed
            ),
            "total_processing_time": round(self._total_processing_time, 1),
            "avg_processing_time": round(avg_time, 1),
            "avg_processing_time_formatted": self.heartbeat._format_duration(avg_time)
            if avg_time > 0
            else "N/A",
            "jobs_per_hour": round(self._jobs_completed / (session_duration / 3600), 2)
    ...
    def _handle_failure(
        ...
        max_retries = get_max_retries()
        logger.warning(
            f"Job {job_hash} FAILED (retry {retry_count}/{max_retries}): {reason}"
        )
        ...
        self.s3.save_job_error_detail(
            job_hash, error=reason, traceback=traceback_str, context=error_context
        )

        if retry_count >= max_retries:
            logger.error(
                f"Job {job_hash} exceeded max retries. Moving to dead-letter queue."
            )
            self.s3.mark_dead(job_hash, reason, retry_count)
            if self.db.available:
                self.db.dead_job(job_hash, reason, retry_count)
            ...
            if self.db.available:
                self.db.fail_job(job_hash, reason, self.id, retry_count)
    ...
    def _init_marker(self):
        ...
        logger.info(
            "Initializing marker models (this may take a while on first run)..."
        )
        try:
    ...
    def _run_conversion_with_timeout(
        self, pdf_path: str, timeout_seconds: int
    ) -> tuple:
        """
        ...
        if not hasattr(signal, "SIGALRM") or not hasattr(signal, "setitimer"):
            logger.warning(
                "conversion_timeout configured but SIGALRM/setitimer is unavailable; running without hard timeout."
            )
            return self._run_marker_conversion(pdf_path)

        if threading.current_thread() is not threading.main_thread():
            logger.warning(
                "conversion_timeout cannot be enforced outside the main thread; running without hard timeout."
            )
            return self._run_marker_conversion(pdf_path)
    ...
    def _requeue_active_job(self, reason: str = "graceful_shutdown") -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Could not update todo marker for {job_hash}: {e}. Falling back to move_to_todo()."
            )
            try:
                ...
                logger.error(
                    f"Failed to requeue active job {job_hash} during shutdown: {fallback_error}"
                )
        ...
        try:
            ...
            logger.warning(
                f"Failed to release lock for {job_hash} during shutdown: {e}"
            )
        ...
        if requeued:
            logger.info(
                f"Re-queued active job {job_hash} (priority={priority}) before shutdown."
            )
        return requeued

    def shutdown(
        self, requeue_current_job: bool = False, reason: str = "graceful_shutdown"
    ):
...
def run_worker_loop(
    worker: Worker, run_once: bool = False, idle_sleep: Optional[float] = None
) -> int:
    """
    ...
    def handle_shutdown_signal(signum, _frame):
        ...
        if shutdown_in_progress:
            logger.warning(
                f"Received {_signal_name(signum)} during shutdown; waiting for cleanup to finish."
            )
            return
