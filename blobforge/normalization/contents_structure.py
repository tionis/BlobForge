"""Additional contents evidence for the next (unreleased) normalization profile.
...
def recover_contents_topics(outline, pages, source_map, report):
    ...
    top-level anchors reject a chapter, not just an inconvenient row.
    """
    ...
    for row in _rows(pages, report.get("toc_pages", [])):
    ...
    for index, major in enumerate(majors):
        ...
        for row, level in zip(rows, levels):
            ...
            if (
                ...
                candidates = [
                    n
                    for n in children
                    if _key(re.sub(r"\s*\([^()]+\)\s*$", "", n["title"])) == row["key"]
        ...
        if any(level == 3 for _, level in unmatched) or len(anchors) < len(rows) * 0.85:
            report["diagnostics"].append(
                f"contents_tiers_alignment_incomplete: {major['title']}: {unmatched}"
            )
            continue
        if sum(level == 3 for level in anchors.values()) < 3:
            continue
        active = 3
    ...
    report["topic_hierarchy"]["method"] = "multi-evidence-contents-v1"
    if recovered:
        report["diagnostics"] = [
            d
