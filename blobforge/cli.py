...
def cmd_reprioritize(args):
    ...
    if s3.exists(f"{S3_PREFIX_PROCESSING}/{job_hash}"):
        print(
            f"Error: Job {job_hash} is currently being processed. Cannot reprioritize."
        )
        return 1
    ...
    if s3.exists(f"{S3_PREFIX_DEAD}/{job_hash}"):
        print(
            f"Error: Job {job_hash} is in dead-letter queue. Use 'retry' command first."
        )
        return 1
    ...
    if not current_prio:
        ...
        if s3.exists(f"{S3_PREFIX_FAILED}/{job_hash}"):
            print(
                f"Error: Job {job_hash} is in failed queue. It will be retried by janitor."
            )
            return 1
...
def cmd_manifest_stats(args):
    ...
    if args.repair:
        ...
        if args.force:
            ...
            if s3.update_manifest(entries, max_retries=20):
                ...
                print(
                    "Repair failed. Try again with --force if no other processes are running."
                )
                return 1
    ...
    if args.rebuild:
        ...
        if args.dry_run:
            print(
                f"[DRY-RUN] Would save manifest with {len(manifest['entries'])} entries"
            )
            return 0
...
def cmd_manifest_sync(args, s3):
    ...
    if args.force:
        ...
        if s3.update_manifest(entries, max_retries=20):
            ...
            print(
                "Sync failed. Try again with --force if no other processes are running."
            )
            return 1
...
def cmd_repair_metadata(args):
    ...
    if args.hashes:
        ...
        print(
            f"Repairing raw metadata for all {len(target_hashes)} manifest entries..."
        )
...
def cmd_workers(args):
    ...
    for w in workers:
        ...
        status_icon = (
            "🟢"
            if status == "active" or status == "processing"
            else "🔴"
            if status == "stopped"
            else "⚪"
        )
        ...
        if jobs_completed > 0 or args.verbose:
            print(
                f"      Jobs: {jobs_completed} completed, {jobs_failed} failed | {jobs_per_hour:.1f}/hr | Avg: {avg_time}"
            )
        ...
        if args.verbose:
            print(
                f"      Platform: {w.get('platform', '?')} {w.get('platform_release', '')}"
            )
            print(f"      Python: {w.get('python_version', '?')}")
            print(
                f"      CPUs: {w.get('cpu_count', '?')}, Memory: {w.get('memory_gb', '?')} GB"
            )
    ...
    if total_completed > 0:
        ...
        print(
            f"  Aggregate: {total_completed} completed, {total_failed} failed, {size_str} processed"
        )
...
def cmd_test_s3(args):
    ...
    try:
        ...
        s3.s3.put_object(
            Bucket=S3_BUCKET, Key=cond_key, Body="first write", IfNoneMatch="*"
        )
        ...
        try:
            ...
            report(
                "If-None-Match: *", False, "Second write succeeded (should have failed)"
            )
            ...
            if error_code in ["PreconditionFailed", "412"]:
                ...
                report(
                    "If-None-Match: *", True, "PreconditionFailed correctly returned"
                )
            else:
                results["conditional_if_none_match"] = False
    ...
    try:
        ...
        s3.s3.put_object(Bucket=S3_BUCKET, Key=etag_key, Body="version 2", IfMatch=etag)
        ...
        try:
            ...
            report(
                "If-Match (ETag)",
                False,
                "Stale ETag update succeeded (should have failed)",
            )
        except s3.ClientError as e:
            error_code = e.response.get("Error", {}).get("Code", "")
            if error_code in ["PreconditionFailed", "412"]:
    ...
    try:
        ...
        report(
            "Delete (DELETE)",
            True,
            f"Cleaned up {len(resp.get('Contents', []))} test objects",
        )
    except Exception as e:
    ...
    conditional_ok = (
        results["conditional_if_none_match"] and results["conditional_if_match"]
    )
    ...
    if basic_ok and conditional_ok:
        ...
        print(
            "   Distributed locking works, but manifest updates may have race conditions."
        )
        print()
        print("   Recommended config:")
        print("     s3_supports_conditional_writes: true")
        print(
            "   Note: Run only one ingestor at a time, or use --force for manifest updates."
        )
    elif basic_ok:
        print("✅ COMPATIBLE (with soft locking)")
        print(
            "   Basic operations work. BlobForge will use timestamp-based soft locking"
        )
    ...
    for job in s3.list_processing():
        ...
        if entry:
            ...
            for path in paths:
                if query in path.lower():
                    results.append(
                        {"hash": job_hash, "path": path, "status": "processing"}
                    )
                    break
...
def cmd_cancel(args):
    ...
    priority = (
        lock_data.get("priority", DEFAULT_PRIORITY) if lock_data else DEFAULT_PRIORITY
    )
...
def cmd_db_init(args):
    ...
    db = DatabaseClient()
    if not db.available:
        print("Error: Database not available. Set BLOBFORGE_DATABASE_URL.")
        return 1
    print("Initializing database schema...")
    db.init_db()
    version = db.current_schema_version()
    print(f"Database initialized. Schema version: {version}")
    return 0
...
def cmd_db_migrate(args):
    ...
    db.run_migrations()
    version = db.current_schema_version()
    print(f"Migrations complete. Schema version: {version}")
    return 0


def cmd_db_status(args):
    """Show database connection and schema status."""
    from .db import DatabaseClient, HAS_PSYCOPG2

    print(f"psycopg2 available: {HAS_PSYCOPG2}")
    print(
        f"BLOBFORGE_DATABASE_URL: {'set' if os.getenv('BLOBFORGE_DATABASE_URL') else 'not set'}"
    )
    db = DatabaseClient()
    print(f"Database connected: {db.available}")
    if db.available:
        version = db.current_schema_version()
        print(f"Schema version: {version}")
        file_count = db.file_count()
        print(f"Files in manifest: {file_count}")
        counts = db.get_queue_counts()
        print("\nQueue state:")
        todo = counts.get("todo", {})
        if isinstance(todo, dict):
            for prio, cnt in todo.items():
                print(f"  todo/{prio}: {cnt}")
...
def cmd_db_migrate_manifest(args):
    """Migrate S3 manifest into the database."""
    from .db import DatabaseClient
    ...
    if not db.available:
        ...
        return 1
    s3 = S3Client()
    print("Fetching manifest from S3...")
    manifest = s3.get_manifest()
    entries = manifest.get("entries", {})
    print(f"Found {len(entries)} entries in manifest")
    count = db.migrate_manifest(manifest)
    print(f"Migrated {count} file entries into the database")
    return 0


def cmd_db_migrate_queues(args):
    """Migrate S3 queue state into the database."""
    from .db import DatabaseClient
    ...
    if not db.available:
        ...
        return 1
    s3 = S3Client()
    print("Migrating queue state from S3...")
    counts = db.migrate_queues(s3)
...
def cmd_db_migrate_all(args):
    """Migrate all S3 state (manifest + queues) into the database."""
    result = cmd_db_migrate_manifest(args)
    if result != 0:
        return result
    return cmd_db_migrate_queues(args)


def main():
    parser = argparse.ArgumentParser(
        prog="blobforge", description="BlobForge - Distributed PDF Conversion System"
    )
    ...
    p_ingest.add_argument(
        "paths",
        nargs="+",
        help="PDF files or directories to ingest (supports shell globbing)",
    )
    p_ingest.add_argument(
        "--priority",
    ...
    p_convert = subparsers.add_parser(
        "convert", help="Convert a PDF file locally (offline)"
    )
    p_convert.add_argument("path", help="Path to the PDF file")
    p_convert.add_argument(
        "--output", "-o", help="Output directory (default: current_dir/filename)"
    )
    p_convert.set_defaults(func=cmd_convert)

    # Hydrate local markdown/assets from completed conversions
    p_hydrate = subparsers.add_parser(
        "hydrate", help="Hydrate local markdown/assets from completed conversions"
    )
    p_hydrate.add_argument(
        "paths", nargs="+", help="PDF files or directories to hydrate"
    )
    p_hydrate.add_argument(
    ...
    # Reprioritize
    p_prio = subparsers.add_parser(
        "reprioritize", help="Change priority of a queued job"
    )
    ...
    p_retry.add_argument(
        "--reset-retries", action="store_true", help="Reset retry counter to 0"
    )
    p_retry.set_defaults(func=cmd_retry)

    # Janitor
    p_janitor = subparsers.add_parser(
        "janitor", help="Run janitor to recover stale jobs"
    )
    ...
    p_worker.add_argument(
        "--dry-run", action="store_true", help="Don't actually modify S3"
    )
    p_worker.add_argument(
        "--run-once", action="store_true", help="Process one job and exit"
    )
    ...
    p_search = subparsers.add_parser(
        "search", help="Search manifest by filename or tag"
    )
    ...
    p_manifest = subparsers.add_parser(
        "manifest", help="Manifest operations: stats, repair, rebuild, sync"
    )
    p_manifest.add_argument(
        "--verbose", "-v", action="store_true", help="Show tag breakdown"
    )
    p_manifest.add_argument(
        "--repair",
        action="store_true",
    ...
    p_repair_metadata = subparsers.add_parser(
        "repair-metadata", help="Restore raw PDF S3 metadata from manifest entries"
    )
    ...
    p_config = subparsers.add_parser(
        "config", help="View or update remote configuration"
    )
    ...
    p_workers.add_argument(
        "--active", action="store_true", help="Show only active workers"
    )
    p_workers.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed info"
    )
    ...
    p_watch.add_argument(
        "--interval", "-i", type=int, default=10, help="Refresh interval in seconds"
    )
    p_watch.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed info"
    )
    ...
    p_download = subparsers.add_parser(
        "download", help="Download completed job results"
    )
    ...
    p_preview.add_argument(
        "--lines", "-n", type=int, default=50, help="Lines of markdown to show"
    )
    p_preview.set_defaults(func=cmd_preview)

    # Retry-all
    p_retry_all = subparsers.add_parser("retry-all", help="Retry all failed/dead jobs")
    p_retry_all.add_argument(
        "--failed", action="store_true", help="Retry failed jobs only"
    )
    p_retry_all.add_argument(
        "--dead", action="store_true", help="Retry dead-letter jobs only"
    )
    p_retry_all.add_argument(
        "--priority",
        default=DEFAULT_PRIORITY,
    ...
    p_retry_all.add_argument(
        "--reset-retries", action="store_true", help="Reset retry counters"
    )
    p_retry_all.add_argument(
        "--dry-run", action="store_true", help="Don't make changes"
    )
    p_retry_all.set_defaults(func=cmd_retry_all)

    # Clear-dead
    p_clear_dead = subparsers.add_parser(
        "clear-dead", help="Clear the dead-letter queue"
    )
    p_clear_dead.add_argument("--force", action="store_true", help="Skip confirmation")
    p_clear_dead.add_argument(
        "--dry-run", action="store_true", help="Don't make changes"
    )
    p_clear_dead.set_defaults(func=cmd_clear_dead)

    # Search-queue
    p_search_queue = subparsers.add_parser(
        "search-queue", help="Search queue by filename"
    )
    p_search_queue.add_argument("query", help="Filename pattern to search")
    p_search_queue.add_argument(
    ...
    p_cancel.add_argument(
        "--priority", choices=PRIORITIES, help="Priority when re-queued"
    )
    ...
    p_remove = subparsers.add_parser(
        "remove", help="Remove a job from queues, raw store, and manifest"
    )
    ...
    # Database Commands
    # =========================================================================
    p_db = subparsers.add_parser("db", help="Database management commands")
    db_sub = p_db.add_subparsers(dest="db_command", required=True)

    db_init = db_sub.add_parser("init", help="Initialize database schema")
    db_init.set_defaults(func=cmd_db_init)

    db_migrate = db_sub.add_parser("migrate", help="Run pending database migrations")
    db_migrate.set_defaults(func=cmd_db_migrate)

    db_status = db_sub.add_parser(
        "status", help="Show database connection and schema status"
    )
    db_status.set_defaults(func=cmd_db_status)

    db_migrate_manifest = db_sub.add_parser(
        "migrate-manifest", help="Migrate S3 manifest into database"
    )
    db_migrate_manifest.set_defaults(func=cmd_db_migrate_manifest)

    db_migrate_queues = db_sub.add_parser(
        "migrate-queues", help="Migrate S3 queue state into database"
    )
    db_migrate_queues.set_defaults(func=cmd_db_migrate_queues)

    db_migrate_all = db_sub.add_parser(
        "migrate-all", help="Migrate all S3 state into database"
    )
    db_migrate_all.set_defaults(func=cmd_db_migrate_all)

    # =========================================================================
    # Telegram Bot
    # =========================================================================
    p_telegram = subparsers.add_parser(
        "telegram", help="Run the Telegram bot for remote management"
    )
